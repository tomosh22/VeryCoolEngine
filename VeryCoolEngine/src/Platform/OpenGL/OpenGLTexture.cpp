...
#pragma region 2d
	...
	void OpenGLTexture2D::PlatformInit()
	{
		...
		if (_initFromFile) {
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mipLevels > 1 ? GL_NEAREST_MIPMAP_LINEAR : GL_NEAREST);
			...
			GLenum wrap = _wrapMode == TextureWrapMode::Clamp ? GL_CLAMP_TO_EDGE : GL_REPEAT;
			GLenum format;
			switch (_format) {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, _width, _height, 0, GL_RGBA, GL_FLOAT, nullptr);//#todo is unsigned byte always ok?
			//#todo float now
			glGenerateMipmap(GL_TEXTURE_2D);
		}
	}
...
#pragma region cube
	...
	void OpenGLTextureCube::PlatformInit()
	{
		glGenTextures(1, &_id);
		glBindTexture(GL_TEXTURE_CUBE_MAP, _id);
		...
		for (auto& face : faces) {
			std::string file = TEXTUREDIR + _filePath + face;
			data = (char*)stbi_load((TEXTUREDIR + _filePath + face).c_str(), &width, &height, &channels, 0);
